...
equalize_eric_script <- function(eric_script) {
  eric_script <- dplyr::mutate(
    eric_script,
    Gene1 = GeneName1,
    Gene2 = GeneName2,
    ...
    SupportingReads = spanningreads,
    Chr1 = equalize_chrom(chr1),
    Break1 = suppressWarnings(as.integer(Breakpoint1)),
    Strand1 = strand1,
    Chr2 = equalize_chrom(chr2),
    Break2 = suppressWarnings(as.integer(Breakpoint2)),
    Strand2 = strand2,

    FMFusionID = character(nrow(eric_script)),
    FMFrameShiftClass = character(nrow(eric_script)),
    FMKnownTranscript1 = character(nrow(eric_script)),
    FMKnownExonNumber1 = character(nrow(eric_script)),
    FMKnownTranscript2 = character(nrow(eric_script)),
    FMKnownExonNumber2 = character(nrow(eric_script)),
    FMFusionJunctionSequence = character(nrow(eric_script))
  )
  return(eric_script)
}
...
equalize_fusion_catcher <- function(fusion_catcher) {
  ...
  fusion_catcher$OrderedFusion <- get_ordered(fusion_catcher$Gene1, fusion_catcher$Gene2)
  fusion_catcher$UnorderedFusion <- get_unordered(fusion_catcher$Gene1, fusion_catcher$Gene2)
  ...
  fusion_catcher$FMFrameShiftClass <- character(nrow(fusion_catcher))
  ...
  fusion_catcher$FMFusionJunctionSequence <- character(nrow(fusion_catcher))
...
equalize_fusion_map <- function(fusion_map) {
  ...
  fusion_map$OrderedFusion <- get_ordered(fusion_map$Gene1, fusion_map$Gene2)
  fusion_map$UnorderedFusion <- get_unordered(fusion_map$Gene1, fusion_map$Gene2)
  ...
  fusion_map$FMFrameShiftClass <- fusion_map$FrameShiftClass
  ...
  fusion_map$FMFusionJunctionSequence <- fusion_map$FusionJunctionSequence
...
equalize_jaffa <- function(jaffa) {
  ...
  jaffa$OrderedFusion <- get_ordered(jaffa$Gene1, jaffa$Gene2)
  jaffa$UnorderedFusion <- get_unordered(jaffa$Gene1, jaffa$Gene2)
  ...
  jaffa$FMFrameShiftClass <- character(nrow(jaffa))
  ...
  jaffa$FMFusionJunctionSequence <- character(nrow(jaffa))
...
equalize_map_splice <- function(map_splice) {
  ...
  map_splice$OrderedFusion <- get_ordered(map_splice$Gene1, map_splice$Gene2)
  map_splice$UnorderedFusion <- get_unordered(map_splice$Gene1, map_splice$Gene2)
  ...
  map_splice$FMFrameShiftClass <- character(nrow(map_splice))
  ...
  map_splice$FMFusionJunctionSequence <- character(nrow(map_splice))
...
equalize_soap_fuse <- function(soap_fuse) {
  ...
  soap_fuse$FMFrameShiftClass <- character(nrow(soap_fuse))
  ...
  soap_fuse$FMFusionJunctionSequence <- character(nrow(soap_fuse))
...
equalize_star_fusion <- function(star_fusion) {
  ...
  star_fusion$OrderedFusion <- get_ordered(star_fusion$Gene1, star_fusion$Gene2)
  star_fusion$UnorderedFusion <- get_unordered(star_fusion$Gene1, star_fusion$Gene2)
  ...
  star_fusion$FMFrameShiftClass <- character(nrow(star_fusion))
  ...
  star_fusion$FMFusionJunctionSequence <- character(nrow(star_fusion))
...
equalize_tophat_fusion <- function(tophat_fusion) {
  ...
  tophat_fusion <- dplyr::mutate(
    ...
    FMFrameShiftClass = character(nrow(tophat_fusion)),
    ...
    FMFusionJunctionSequence = character(nrow(tophat_fusion))
  )
...
equalize_dragen <- function(dragen) {
  ...
  dragen$FMFrameShiftClass <- character(nrow(dragen))
