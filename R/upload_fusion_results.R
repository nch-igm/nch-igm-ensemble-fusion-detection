...
################################################################################ 
#Fusion detection algorithms compatible with: fusionMap, fusionCatcher, jaffa,
# mapSplice, soapFuse, starFusion, tophatFusion, DRAGEN
################################################################################ 
...
description <- paste0("This script acts as an importer for fusion detection results to the fusion detection database.")
option_list <- list(
  make_option("--subject", type = "character", help = "Subject ID, e.g. subject1. (required)"), 
  make_option("--sample", type = "character", help = "Sample ID, e.g. sample1. (required)"), 
  make_option("--phenotype", type = "character", help = "Phenotype of the subject. (optional)"), 
  make_option("--storage", type = "character", help = "Storage method for the sample, such as 'Frozen' or 'FFPE'. (optional)"),
  make_option("--reads", type = "integer", help = "Number of read pairs for the sample. (optional)"), 
  make_option("--tool", type = "character", help = "Custom tool for which the results are being entered, possible values are 'dragen', 'FusionCatcher', 'FusionMap', 'Jaffa', 'MapSplice', 'SoapFuse', 'StarFusion', and 'TophatFusion'. (optional)"),
  make_option("--results", type = "character", help = "Custom location of the results file for the associated tool, e.g. 'star-fusion.fusion_predictions.abridged.tsv'. (optional)")
)
